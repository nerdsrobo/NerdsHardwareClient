<script lang="ts">
    ...
    const props: { apis: APIs, goto: Function } = $props();
    const apis = props.apis;

    // const manageTab = new Tab(() => window.innerWidth, "manage", true);
    const manageTab = new Tab(() => window.innerWidth, "manage", false);
    const dashboardTab = new Tab(() => window.innerWidth, "dashboard", true);

    const manageArrowAngle = $derived(manageTab.openingOrOpen ? 90 : -90);
    const dashboardArrowAngle = $derived(dashboardTab.openingOrOpen ? 90 : -90);

</script>

<!-- svelte-ignore a11y_click_events_have_key_events -->
<div class="left-panel" style={apis.settingsApi.settings.display_dash ? "grid-template-rows: 85px 1px 1fr 1px 1fr" : "grid-template-rows: 85px 1px 1fr"}>
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="left-panel-manage cursor-pointer" onclick={() => {manageTab.handleClick()}}>
    ...
    {#if apis.settingsApi.settings.display_dash }
        <div class="left-panel-line2 panel-line"></div>
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <div class="left-panel-dashboard cursor-pointer" onclick={() => {dashboardTab.handleClick()}}>
